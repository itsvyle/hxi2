...
def extract_folders_files(s: BeautifulSoup):
    ...
    for p in ps:
        ...
        if p.attrs["class"] == ["rep"]:
            ...
            fext = fext.split(",")[0]
            nom = enleve_espace_fin(
                rendre_nom_valide(p.find(class_="nom").string + "." + fext)
            )
...
def rec_extract_folders_files(url, root=""):
    ...
    def aux(
        fp, url
    ):  # en parcours "postfixe" pour que recréer les dossiers soit possible en dépilant
        req = urlr.Request(f"{CDPLINK}/docs{url}", headers={"Cookie": COOKIES})
...
def download_and_save(fp, url):
    req = urlr.Request(f"{CDPLINK}/{url}", headers={"Cookie": COOKIES})

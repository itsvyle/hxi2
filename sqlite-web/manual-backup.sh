...
if [ -z "$SQLITE_WEB_FILES_CONFIG" ]; then
    echo "Error: SQLITE_WEB_FILES_CONFIG is not set."
    exit 1
fi
...
echo "Writing backup to $outputPath"

uv run /app/backup.py -o "$outputPath" "$SQLITE_WEB_FILES_CONFIG" "$SQLITE_BACKUP_PUBLIC_KEY"

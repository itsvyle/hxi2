...
pub fn get_route_from_public_url(url: &str) -> Option<&'static str> {
	if url == "/auth.v2.AuthService/GetCSRFToken" { return Some("/auth.v2.AuthService/GetCSRFToken") }
...
impl CompiledPermissions {
    pub fn get_by_route(&self, route: &str) -> Option<&MethodPermissions> {
        match route {
				"/auth.v2.AuthService/GetCSRFToken" => return Some(&self.permissions[0].1),
				"/auth.v2.AuthService/GetDevToken" => return Some(&self.permissions[1].1),
				"/auth.v2.AuthService/GetJWTPublicKey" => return Some(&self.permissions[2].1),
				"/auth.v2.AuthService/ListUsers" => return Some(&self.permissions[3].1),
				"/auth.v2.AuthService/Login" => return Some(&self.permissions[4].1),
				"/auth.v2.AuthService/RenewJWT" => return Some(&self.permissions[5].1),
            _ => None,
        }
    }
}
